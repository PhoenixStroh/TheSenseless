...
public class CardForget extends AbstractDynamicCard {
    ...
    public static final String ID = SenselessMod.makeID("CardForget"); // DELETE THIS ONE.
    ...
    private static final int DISCARD_TOTAL = 3;
    
    private static final int SCRY_TOTAL = 1;
    private static final int UPGRADE_PLUS_SCRY_TOTAL = 2;

    // /STAT DECLARATION/


    public CardForget() { // public ${NAME}() - This one and the one right under the imports are the most important ones, don't forget them
        super(ID, IMG, COST, TYPE, COLOR, RARITY, TARGET);

        baseMagicNumber = magicNumber = SCRY_TOTAL;
    }
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        AbstractDungeon.actionManager.addToBottom(new UpToDiscardAction(DISCARD_TOTAL, true));
    ...
    public void upgrade() {
        if (!upgraded) {
            ...
            upgradeMagicNumber(UPGRADE_PLUS_SCRY_TOTAL);
